...
class ProblemDescription(Scene):
    ...
    def get_angle_between_vectors(self, wall_angle: list, vec1: list, vec2: list) -> float:
        # Recalculate vectors after applying wall_angle
        vec1 = np.array([vec1[0] - wall_angle[0], vec1[1] - wall_angle[1]])
        vec2 = np.array([vec2[0] - wall_angle[0], vec2[1] - wall_angle[1]])

        # Get scalar multiplic
        scalar_multiplic = np.prod(vec1) + np.prod(vec2)

        # Get vectors' modules
        vec1_mod = sqrt(vec1[0] ** 2 + vec1[1] ** 2)
        vec2_mod = sqrt(vec2[0] ** 2 + vec2[1] ** 2)

        # Get angle in radians
        return np.arccos(scalar_multiplic / (vec1_mod * vec2_mod))

    def create_guard_view(self, guard: Dot, gallery: Polygon) -> list:
        view_points_coords = []
        ...
        for wall_num, wall_angle in enumerate(gallery_corners):
            ...
            if self.is_segment_inside_polygon(help_line, gallery):
                ...
                # Проверяем, видит ли охранник точку за углом
                # (проверяем для всех углов, даже меньше 180)
                ...
                # Находим и конвертируем в точки пересечения
                ...
                if not intersection.is_empty:
                    ...
                    for ip in ips:
                        ...
                        if self.is_segment_inside_polygon(
                            ...
                            if not len(view_points_coords) == 0:
                                # Указываем правильный порядок обхода
                                last_visible = view_points_coords[
                                    len(view_points_coords) - 1 % len(view_points_coords)
                                ]

                                if gallery_corners[wall_num - 1 % len(view_points_coords)] not in view_points_coords:
                                    connected = gallery_corners[wall_num - 1 % len(view_points_coords)].get_center()
                                else:
                                    connected = gallery_corners[wall_num + 1 % len(view_points_coords)].get_center()


                                if self.get_angle_between_vectors(
                                    wall_angle, last_visible, connected
                                ) < self.get_angle_between_vectors(
                                    wall_angle, last_visible, possible[:2]
                                ):
    ...
    def construct(self):
        ...
        # Отрисовка охранника
        guard = Dot([-2.5, -1, 0], 0.12, color=PURE_RED)
        self.play(Create(guard))

        self.wait()

        self.add(*[Dot(i, color=RED) for i in self.create_guard_view(guard, polygon)])
                ...
                guard_view_shapely_line = LineString(
                    [
                        ...
                        shapely_scale(
                            ...
                            xfact=max_calc_dist,
                            yfact=max_calc_dist,
                            wall_angle=shapely_guard.coords[0],
                        ).coords[0],
                    ]
                )
"""
